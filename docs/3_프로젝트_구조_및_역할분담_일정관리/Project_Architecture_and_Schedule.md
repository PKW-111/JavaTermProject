...
## 1. 문서 목적 및 범위
본 문서는 **2주차 산출물(요구사항 분석 및 설계)**을 기반으로 FreshKeeper의 구조를 구현 관점으로 재정의하고, 팀 운영 방식과 **6월 11일 마감 기준 주차별 일정**(마일스톤/점검 기준 포함)을 명확히 제시한다.
...
## 4. 일정 관리 계획 (6월 11일 마감 기준)

## 4.1 주차별 실행 계획

| 주차 | 기간 | 목표 | 세부 작업 | 산출물 | 완료 판정 |
|---|---|---|---|---|---|
| 1주차 | 05.12 ~ 05.18 | 설계 고도화/기준선 확정 | 요구사항 재검토, 클래스 책임 확정, 화면/시퀀스 문서화 | 구조 문서, 역할표, 일정표 초안 | 요구사항-모듈 매핑 100% 작성 |
| 2주차 | 05.19 ~ 05.25 | 핵심 기능 1차 구현 | 등록/조회/삭제, 유통기한 계산/분류 구현 | 핵심 기능 동작 버전 | F-01~F-05 시나리오 통과 |
| 3주차 | 05.26 ~ 06.01 | 추천/예외 처리 강화 | 추천/부족재료, 입력 예외 처리 보강 | 추천 기능 + 안정성 개선본 | F-06~F-08 시나리오 통과 |
| 4주차 | 06.02 ~ 06.08 | 데이터 지속성/통합 안정화 | 저장/불러오기 구현, 통합 테스트, 회귀 점검 | 통합 테스트 결과표, 결함 수정본 | F-09 반영 + 주요 결함 정리 |
| 5주차 | 06.09 ~ 06.11 | 제출 패키지 완성 | 최종 리팩터링, 문서/발표자료 정리, 제출본 동결 | 최종 코드 + 최종 문서 + 발표자료 | 코드-문서-발표자료 일치 |

## 4.2 마일스톤 및 중간 점검

| 마일스톤 | 시점 | 점검 항목 | 점검 결과 기록 |
|---|---|---|---|
| M1 설계 동결 | 1주차 말(05.18) | 요구사항-구조 매핑, 클래스 책임 중복 여부 | 회의 기록 + 문서 버전 |
| M2 핵심 기능 1차 | 2주차 중간(05.22) | 등록/조회/삭제, 유통기한 계산 데모 | 데모 캡처/실행 로그 |
| M3 핵심 기능 2차 | 2주차 말(05.25) | 상태 분류 정확도, 기본 흐름 안정성 | 이슈 리스트(1차) |
| M4 추천/예외 처리 완료 | 3주차 말(06.01) | 추천/부족재료, 입력 예외 처리 검증 | 이슈 리스트(2차) |
| M5 지속성/통합 완료 | 4주차 말(06.08) | 저장/복원, 통합/회귀 테스트 | 테스트 결과표 |
| M6 문서/발표자료 동결 | 5주차 초(06.10) | 코드-문서-발표자료 정합성 | 제출 점검표 |
| M7 최종 제출 | 5주차 말(06.11) | 최종 패키지 제출 | 제출본 |
...
| W10 | 제출본 동결/최종 제출 | W9 | 박경우(리드) | 최종 제출 패키지 |

## 4.4 리스크 및 대응 계획

| 리스크 | 조기 징후(Trigger) | 영향 | 대응 방안 |
|---|---|---|---|
| 기능 구현 지연 | 2주차 중간 데모 불가 | 통합 일정 지연 | 핵심 요구사항 우선순위 재조정 + 5주차 버퍼 활용 |
...
| 일정 계획의 현실성 | 6월 11일 기준 5주 계획 + 마일스톤 + WBS + 리스크 대응 | 4.1 ~ 4.4 |
| 문서 정리 완성도 | 표 기반 가독성, 용어 통일, 평가 기준 직접 대응 | 전 섹션 |

## 6. 결론
FreshKeeper는 요구사항 기반 구조(기능-모듈 추적 가능), 책임이 분명한 역할 분담, 그리고 **6월 11일 마감에 맞춘 5주 점검 중심 일정 계획**을 통해 실제 구현 가능성과 평가 대응력을 동시에 확보한다.
